# \# Password Manager (WPF)

# 

# Password Manager is a simple desktop application created as a high school graduation (maturita) project. The application is used to store and manage login credentials for different services, with focus on easy use.

# 

# ---

# \## Project Description

# 

# The application allows users to create and manage accounts, organise them into folders, mark accounts as favourites, and search stored data.

# The project focuses on desktop UI development, data model design, and application logic using .NET and WPF.

# 

# ---

# \## Project Goal

# 

# The goal of this project was to create a fucntional desktop application using WPF that demonstrates:

# \- work with data models and collections

# \- user interface design

# \- basic application logic

# 

# ---

# \## Features

# 

# \- create, edit, and delete accounts

# \- folder management

# \- view all accounts (All Accounts)

# \- assign accounts to folders

# \- mark accounts as favourites and sort them

# \- search by service name and note

# \- confirmation dialogs for delete actions

# 

# ---

# \## Technologies Used

# 

# \- C#

# \- .NET (WPF)

# \- XAML

# \- ObservableCollection

# \- INotifyPropertyChanged

# 

# Developed using Visual Studio

# 

# ---

# \## How to Run

# 

# 1\. Open the project in Visual Studio

# 2\. Run the application by pressing \*\*F5\*\*

# 

# ---

# \## Author

# 

# Stanislav Kuča

# Střední průmyslová škola, Obchodní akademie a Jazyková škola s právem státní jazykové zkoušky, Frýdek≈Místek, příspěvková organizace

# 2025/26



